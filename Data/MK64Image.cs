...
namespace MK64Pitstop.Data
{
    public class MK64Image : IXMLSerializable
    {
        ...
        /// </summary>
        public int TextureBlockOffset { get; private set; }
        ...
        /// <summary>
        /// The actual texture used in the image
        /// </summary>
        ...
        /// <summary>
        /// Offset within the ROM for the palette data
        /// </summary>
        public int PaletteOffset { get; private set; }

        /// <summary>
        /// Offset within the palette block for the palette data (for encoded values)
        /// </summary>
        public int PaletteBlockOffset { get; private set; }

        /// <summary>
        /// The actual palette used in the image
        /// </summary>
        public Palette PaletteReference { get; private set; }

        /// <summary>
        /// Byte encoding of the palette data
        /// </summary>
        public MK64ImageEncoding PaletteEncoding { get; private set; }
        ...
        /// <summary>
        /// Offset to use in the palette
        /// </summary>
        public int PaletteColorOffset { get; private set; }
        ...
        public Bitmap Image
        {
            get
            {
                if ((TextureReference == null || TextureReference.Image == null) && (TKMKReference == null || TKMKReference.Image == null))
                    return null;

                if (TKMKReference != null && TKMKReference.Image != null)
                    return TKMKReference.Image;

                return TextureReference.Image;
            }
            //set {}
        }
        ...
        //Special constructor for the MK64ImageInfo class
        public MK64Image(MarioKartRomInfo.MK64ImageInfo info, byte[] rawFileData = null)
            : this(info.TextureOffset, (MK64ImageEncoding)Enum.Parse(typeof(MK64ImageEncoding), info.TextureEncoding), info.TextureBlockOffset,
                    ...
                    info.Width, info.Height, info.IsOriginal, info.PaletteOffset, (MK64ImageEncoding)Enum.Parse(typeof(MK64ImageEncoding), info.PaletteEncoding),
        ...
        public MK64Image(int textureOffset, MK64ImageEncoding textureEncoding, int textureBlockOffset, Texture.ImageFormat format,
            ...
            int paletteOffset, MK64ImageEncoding paletteEncoding, int paletteBlockOffset, int paletteColorCount, int paletteColorOffset,
        ...
        public MK64Image(XElement xml)
        {
            ...
            PaletteOffset = int.Parse(xml.Attribute(PALETTE_OFFSET).Value.ToString());
            PaletteEncoding = (MK64ImageEncoding)Enum.Parse(typeof(MK64ImageEncoding), xml.Attribute(PALETTE_ENCODING).Value.ToString());
            PaletteBlockOffset = int.Parse(xml.Attribute(PALETTE_BLOCK_OFFSET).Value.ToString());
            PaletteColorCount = int.Parse(xml.Attribute(PALETTE_COLOR_COUNT).Value.ToString());
            PaletteColorOffset = int.Parse(xml.Attribute(PALETTE_COLOR_OFFSET).Value.ToString());
        ...
        public XElement GetAsXML()
        {
            ...
            xml.Add(new XAttribute(PALETTE_OFFSET, PaletteOffset));
            xml.Add(new XAttribute(PALETTE_BLOCK_OFFSET, PaletteBlockOffset));
            xml.Add(new XAttribute(PALETTE_ENCODING, PaletteEncoding));
            xml.Add(new XAttribute(PALETTE_COLOR_COUNT, PaletteColorCount));
            xml.Add(new XAttribute(PALETTE_COLOR_OFFSET, PaletteColorOffset));
        ...
        private void LoadImageData(byte[] rawData = null)
        {
            ...
            int maxTextureLocation = RomProject.Instance.Files[0].FileLength - 1;

            if(TextureOffset < 0 || TextureOffset > maxTextureLocation ||
                (Format == Texture.ImageFormat.CI && (PaletteOffset < 0 || PaletteOffset > maxTextureLocation)))
                    return;
            ...
            {
                element = RomProject.Instance.Files[0].GetElementAt(PaletteOffset);
                if (element != null && element.FileOffset == PaletteOffset && !(element is UnknownData))
                {
                    switch (PaletteEncoding)
                    {
                        ...
                        case MK64ImageEncoding.MIO0:
                            ...
                            if (block.Elements.FirstOrDefault(e => e.FileOffset == PaletteBlockOffset) != null)
                            {
                                N64DataElement paletteEl = block.Elements.First(e => e.FileOffset == PaletteBlockOffset);
                            ...
                            {
                                //Create the palette!
                                byte[] data = new byte[PaletteColorCount * 2];
                                Array.Copy(block.DecodedData, PaletteBlockOffset, data, 0, data.Length);
                                Palette pal = new Palette(PaletteBlockOffset, data);
                ...
                else
                {
                    ...
                    switch (PaletteEncoding)
                    {
                        case MK64ImageEncoding.Raw:
                            ...
                            //Create the palette!
                            byte[] data = new byte[PaletteColorCount * 2];
                            Array.Copy(rawData, PaletteOffset, data, 0, data.Length);

                            Palette pal = new Palette(PaletteOffset, data);
                        ...
                        case MK64ImageEncoding.MIO0:
                            ...
                            MIO0Block block = MIO0Block.ReadMIO0BlockFrom(rawData, PaletteOffset);
                            if (RomProject.Instance.Files[0].AddElement(block))
                            {
                                //Create the palette in the MIO0 block!
                                byte[] mioData = new byte[PaletteColorCount * 2];
                                Array.Copy(block.DecodedData, PaletteBlockOffset, mioData, 0, mioData.Length);

                                Palette palette = new Palette(PaletteBlockOffset, mioData);
                    ...
                    case MK64ImageEncoding.MIO0:
                        ...
                        else
                        {
                            ...
                            if (Format == Texture.ImageFormat.CI)
                                text = new Texture(TextureBlockOffset, data, Format, PixelSize, Width, Height, paletteRef, PaletteColorOffset);
                            else
                                text = new Texture(TextureBlockOffset, data, Format, PixelSize, Width, Height);
            ...
            else //NEED TO CREATE IT HERE!
            {
                ...
                switch (TextureEncoding)
                {
                    case MK64ImageEncoding.Raw:
                        ...
                        Texture texture;
                        if (Format == Texture.ImageFormat.CI)
                            texture = new Texture(TextureOffset, data, Format, PixelSize, Width, Height, paletteRef, PaletteColorOffset);
                        else
                            texture = new Texture(TextureOffset, data, Format, PixelSize, Width, Height);
                    ...
                    case MK64ImageEncoding.MIO0:
                        ...
                        if (RomProject.Instance.Files[0].AddElement(block))
                        {
                            ...
                            if (Format == Texture.ImageFormat.CI)
                                text = new Texture(TextureBlockOffset, textData, Format, PixelSize, Width, Height, paletteRef, PaletteColorOffset);
                            else
                                text = new Texture(TextureBlockOffset, textData, Format, PixelSize, Width, Height);
            ...
            if((textureRef == null && tkmkRef == null) || (Format == Texture.ImageFormat.CI && paletteRef == null))
                return;
            ...
            //NOTE NOTE NOTE:::: HERE"S A BIG PROBLEM!! IF WE WANT TEXTURES TO BE MATCHED UP WTIH MULTIPLE PALETTES, THIS HAS TO BE RESOLVED!!
            if (textureRef != null && Format == Texture.ImageFormat.CI && textureRef.ImagePalette != paletteRef)
                textureRef.ImagePalette = paletteRef;
